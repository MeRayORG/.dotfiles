...
let
  inherit (builtins) attrNames readDir baseNameOf toString;
  inherit (lib.strings) hasSuffix removeSuffix;
  inherit (lib.filesystem) pathIsDirectory;

  # The recursive importer function:
  itemParser = importFunction: { fileODirName, oldPath, oldImporterPath }:
    ...
    in if isDir then doLoop else (if isDef then parseDef else (if isNix then parseNix else {}));

in
  if !(builtins.isPath dir)
  then throw "importer needs a path"
  else
